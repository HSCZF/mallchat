 ...
 * AbstractRedisStringCache是一个抽象类，它实现了BatchCache接口，并且针对Redis缓存系统进行了具体的实现。
 * UserSummaryCache是AbstractRedisStringCache的一个具体子类，它专门用于处理用户摘要信息的缓存。
 * @Author: CZF
 * @Create: 2024/7/18 - 15:30
 */
public abstract class AbstractRedisStringCache<IN, OUT> implements BatchCache<IN, OUT> {
    ...
    public Map<IN, OUT> getBatch(List<IN> req) {
        ...
        //  相当于
        //  Map<IN, OUT> batch = userSummaryCache.getBatch(uidList);
        ...
        if (CollectionUtil.isNotEmpty(loadReqs)) {
             ...
             * AbstractRedisStringCache是一个抽象类，它实现了BatchCache接口，并且针对Redis缓存系统进行了具体的实现。
             * UserSummaryCache是AbstractRedisStringCache的一个具体子类，它专门用于处理用户摘要信息的缓存。
             * UserSummaryCache的load方法刚好是load方法，所以可以直接使用。
             */
            //批量load
            load = load(loadReqs);
